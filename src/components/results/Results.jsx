...
export const Results = ({ numberOfQuestions, score, missed, showMissedQuestion, refreshPage }) => {
    ...
    useEffect(() => {
        const getKeywords = () => {
            // Step 1: Extract all keyword arrays from missed questions
            const keywordArrays = showMissedQuestion.map((q) => q.keyword);
            console.log('keywordArrays', keywordArrays)
                        ...
                        </div>
                        {
                            showMissedQuestion.map((ques, idx) => (
                                <div className="summary_display" key={idx}>
                                    ...
                                    {ques.image ? <img src={ques.image} alt="" /> : ""}
